...
|─startup_gd32f407_427.s
  |_int main(void)
      ...
      |  |_appDrv8424Init()                         "Drv8424初始化(剪刀夹子)"
      |  |_appDrv8462Init()                         "Drv8462初始化(升降)" 
      |  |_appMps6602Init()                         "MPS6602初始化(拉线)" 
...
#### StepMove
~~~
bsp_Time1Step_init();bsp_Time2Step_init(); bsp_Time4Step_init();
bsp_Time5Step_init();bsp_Time6Step_init(); bsp_Time9Step_init();
bsp_Time10Step_init();bsp_Time11Step_init();bsp_Time12Step_init();
9个定时器开启的定时中断优先级均是最高与CAN接收一样
~~~
![alt text](image.png)
~~~JSON
